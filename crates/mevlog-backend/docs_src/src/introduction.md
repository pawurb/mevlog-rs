...
- **Generate EVM tracing insights** - replay any transaction to extract storage state diffs, decoded call traces, and direct coinbase payments. Tracing runs either through a node's standard `debug_traceTransaction` RPC, or fully locally via [Revm](https://github.com/bluealloy/revm) - no tracing-enabled RPC required.
...
1. **Index** - fetch a block range from an RPC endpoint (using [cryo](https://github.com/paradigmxyz/cryo) under the hood).
...
These docs walk through each step. See [Quick Start](./quick-start.md) to get set up and run your first query.
