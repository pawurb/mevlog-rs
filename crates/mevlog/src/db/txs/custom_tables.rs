...
/// Atomic so a crash mid-backfill can't leave a fingerprint row pointing at an
/// empty/partial table (which `sync` would then accept as valid). Create, meta
/// write, and backfill commit together or not at all — SQLite DDL is
/// transactional.
