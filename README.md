...
**mevlog** is an open-source (MIT) alternative to commercial blockchain indexers and data APIs. Instead of paying for a hosted service and querying someone else's database, it downloads raw chain data straight from an RPC endpoint, stores it in a local SQLite database, and lets you query it with plain SQL. **You own the data** - no API keys, no rate limits, no vendor lock-in, just a file on your disk you can query as much as you want, offline.
