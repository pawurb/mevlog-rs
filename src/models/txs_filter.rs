...
mod tests {
    ...
    fn test_parse_query() {
        ...
        assert!(matches!(op, DiffOperator::GreaterOrEq), "Should be GreaterOrEq operator");
        assert_eq!(value, U256::from(1000000000), "Should parse raw wei value correctly");

        // Test with gwei values
        let (op, value) = parse_query("ge5gwei").unwrap();
        assert!(matches!(op, DiffOperator::GreaterOrEq), "Should be GreaterOrEq operator");
        assert_eq!(value, U256::from(5_000_000_000_u128), "Should convert 5 gwei to wei correctly");

        // Test with ether values
        let (op, value) = parse_query("le0.01ether").unwrap();
        assert!(matches!(op, DiffOperator::LessOrEq), "Should be LessOrEq operator");
        assert_eq!(value, U256::from(10).pow(U256::from(16)), "Should convert 0.01 ether to wei correctly");
    ...
    fn test_tx_cost_query_from_str() {
        ...
        assert!(matches!(query.operator, DiffOperator::GreaterOrEq), "Should parse GreaterOrEq operator");
        assert_eq!(query.diff, U256::from(5_000_000_000_u128), "Should parse 5 gwei correctly");
        
        // Test with ether
        let query = TxCostQuery::from_str("le0.1ether").unwrap();
        assert!(matches!(query.operator, DiffOperator::LessOrEq), "Should parse LessOrEq operator");
        assert_eq!(query.diff, U256::from(10).pow(U256::from(17)), "Should parse 0.1 ether correctly");
        
        // Test with raw wei
        let query = TxCostQuery::from_str("ge1000000").unwrap();
        assert!(matches!(query.operator, DiffOperator::GreaterOrEq), "Should parse GreaterOrEq operator");
        assert_eq!(query.diff, U256::from(1000000), "Should parse raw wei value correctly");
    }
    ...
    fn test_gas_price_query_from_str() {
        ...
        assert!(matches!(query.operator, DiffOperator::GreaterOrEq), "Should parse GreaterOrEq operator");
        assert_eq!(query.gas_price, U256::from(10_000_000_000_u128), "Should parse 10 gwei correctly");
        
        // Test with ether (unusual but should work)
        let query = GasPriceQuery::from_str("le0.000001ether").unwrap();
        assert!(matches!(query.operator, DiffOperator::LessOrEq), "Should parse LessOrEq operator");
        assert_eq!(query.gas_price, U256::from(10).pow(U256::from(12)), "Should parse 0.000001 ether correctly");
    }
    ...
    fn test_matches_functionality() {
        ...
        // Test greater than
        assert!(tx_cost.matches(U256::from(6_000_000_000_u128)), "Should match when value is greater than threshold");
        
        // Test equal
        assert!(tx_cost.matches(U256::from(5_000_000_000_u128)), "Should match when value is equal to threshold");
        
        // Test less than
        assert!(!tx_cost.matches(U256::from(4_000_000_000_u128)), "Should not match when value is less than threshold");
        ...
        // Test greater than
        assert!(!gas_price.matches(U256::from(11_000_000_000_u128)), "Should not match when value is greater than threshold");
        
        // Test equal
        assert!(gas_price.matches(U256::from(10_000_000_000_u128)), "Should match when value is equal to threshold");
        
        // Test less than
        assert!(gas_price.matches(U256::from(9_000_000_000_u128)), "Should match when value is less than threshold");
    }
}