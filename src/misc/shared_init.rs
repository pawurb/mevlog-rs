...
impl MEVChain {
    pub fn new(chain_id: u64, rpc_url: String) -> Result<Self> {
        if chain_id == MEVChainType::Mainnet.chain_id() {
            Ok(Self {
                rpc_url,
                chain_type: MEVChainType::Mainnet,
            })
        } else if chain_id == MEVChainType::Base.chain_id() {
            Ok(Self {
                rpc_url,
                chain_type: MEVChainType::Base,
            })
        } else {
...
pub async fn init_deps(conn_opts: &ConnOpts) -> Result<SharedDeps> {
    ...
    let chain = MEVChain::new(chain_id, conn_opts.rpc_url.clone().unwrap())?;
